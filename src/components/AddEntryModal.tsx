...
const AddEntryModal: React.FC<AddEntryModalProps> = ({ 
  ...
  const submitAction = async (prevState: any, formData: FormData) => {
    ...
    else if (logFormType === LogType.TEMPERATURE) value = (animal.category === AnimalCategory.EXOTICS) ? `B: ${tempBasking}°C C: ${tempCool}°C` : `${tempAmbient}°C`;
        ...
        selectedEventAnimals.forEach(targetId => {
            ...
            if (targetAnimal) {
                const entryForAnimal = {
                    ...baseEntry,
                    id: targetId === animal.id ? baseEntry.id : `ev_${Date.now()}_${targetId}`
                };
  ...
  // Effect hooks for calculations and weather sync...
  useEffect(() => {
    ...
    <div className="fixed inset-0 bg-black/0 flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <form action={formAction} className="p-6 space-y-5 overflow-y-auto">
                    ...
                    {/* ... (Existing form sections mostly unchanged logic-wise) ... */}
                    {/* Just ensuring they use the local state which is then used by submitAction */}
