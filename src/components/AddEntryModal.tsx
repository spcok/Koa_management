    ...
    <div className="fixed inset-0 bg-black/0 flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
            ...
            <form action={formAction} className="p-6 space-y-5 overflow-y-auto">
                    ...
                    {logFormType === LogType.FEED && (<div className="space-y-4 bg-amber-50 p-4 rounded-lg border border-amber-200"><div className="grid grid-cols-2 gap-4"><div><label>Quantity<input type="number" step="0.1" required value={feedQuantity} onChange={e => setFeedQuantity(e.target.value)} className={inputClass}/></label></div><div><label>Food Item<select required value={feedType} onChange={e => setFeedType(e.target.value)} className={inputClass}><option value="">Select...</option>{(foodOptions[animal.category] || []).map(opt => <option key={opt} value={opt}>{opt}</option>)}</select></label></div></div>{(animal.category === AnimalCategory.RAPTORS || animal.category === AnimalCategory.OWLS) && (<div><label>Cast Produced?<select required value={hasCast} onChange={e => setHasCast(e.target.value)} className={inputClass}><option value="">Select...</option><option value="yes">Yes</option><option value="no">No</option></select></label></div>)}</div>)}
                    ...
                    {logFormType === LogType.HEALTH && (<div className="space-y-4 bg-rose-50 p-4 rounded-lg border border-rose-200"><label>Type<select value={healthType} onChange={e => setHealthType(e.target.value as any)} className={inputClass}>{Object.values(HealthRecordType).map(t=><option key={t} value={t}>{t}</option>)}</select></label><label>Body Condition<BCSSelector value={healthBcs} onChange={setHealthBcs} /></label></div>)}
