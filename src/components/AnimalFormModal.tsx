...
const AnimalFormModal: React.FC<AnimalFormModalProps> = ({ isOpen, onClose, onSave, initialData, locations = [] }) => {
  // Local state for controlled inputs that don't need immediate action dispatch
  ...
  const submitAction = async (prevState: any, formDataObj: FormData) => {
    // Process form data here if needed, or rely on state.
    // In a real Server Action, we'd parse formDataObj.
    // For client-side simulation, we use the state `formData` but wrap in action pattern.
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/0 flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl max-h-[95vh] flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden">
            ...
            <form action={formAction} className="flex-1 overflow-y-auto p-8 space-y-12 scrollbar-hide bg-white">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-8 pt-12 border-t border-slate-100 pb-4">
                    ...
                    <div className="flex gap-4 w-full sm:w-auto">
                        ...
                        <button type="submit" disabled={isPending} className="flex-1 sm:flex-none px-12 py-4 bg-[#10b981] hover:bg-[#059669] text-white rounded-xl font-black uppercase text-[10px] tracking-[0.2em] shadow-xl shadow-emerald-900/10 transition-all flex items-center justify-center gap-3 active:scale-95 disabled:opacity-50">
                            {isPending ? <Loader2 size={20} className="animate-spin"/> : <Check size={20} />}
                            {isPending ? 'AUTHORIZING...' : 'AUTHORIZE RECORD'}
                        </button>
                    </div>
                </div>
            </form>
