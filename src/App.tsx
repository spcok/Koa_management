...
import { Animal, AnimalCategory, Task, User, UserRole, SiteLogEntry, Incident, FirstAidLogEntry, OrganizationProfile, Contact, SortOption, TimeLogEntry, UserPermissions, AuditLogEntry } from './types';
  ...
  return (
    <div style={{ fontSize: `${fontScale}%` }}>
      <Layout activeView={view} onNavigate={setView} currentUser={currentUser} onLogout={handleLogout} isOffline={isOffline} fontScale={fontScale} setFontScale={setFontScale} activeShift={activeShift} onClockIn={handleClockIn} onClockOut={handleClockOut} orgProfile={orgProfile}>
        ...
        {view === 'reports' && p.reports && <Reports animals={animals} orgProfile={orgProfile} users={users} currentUser={currentUser} incidents={incidents} siteLogs={siteLogs} timeLogs={timeLogs} firstAidLogs={firstAidLogs} />}
