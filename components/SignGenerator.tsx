                                ...
                                <div className="col-span-7 flex flex-col gap-5">
                                     ...
                                     <div className="grid grid-cols-2 gap-x-8 gap-y-4 mt-auto">
                                         ...
                                         {(animal.category === AnimalCategory.MAMMALS || animal.category === AnimalCategory.EXOTICS) ? (
                                             ...
                                             <div><h4 className="text-[9px] font-black text-slate-300 uppercase tracking-widest">CHIP/RING</h4><p className="font-black text-slate-800 font-mono text-sm">{animal.microchip || animal.ringNumber || 'N/A'}</p></div>
                                         )}
                                     </div>
                                </div>
                                     ...
                                     <div className="flex flex-col gap-3">
                                         <div className="flex gap-3">
                                             <div className="flex-1 bg-orange-500 text-white p-3 rounded-xl shadow-lg flex items-center gap-3">
                                                 ...
                                                 <div className="min-w-0"><p className="text-[8px] font-black opacity-80 uppercase tracking-widest whitespace-nowrap">DAY TARGET</p><p className="text-2xl font-black leading-none">{animal.targetDayTemp || 28}°C</p></div>
                                             </div>
                                             <div className="flex-1 bg-emerald-600 text-white p-3 rounded-xl shadow-lg flex items-center gap-3">
                                                 ...
                                                 <div className="min-w-0"><p className="text-[8px] font-black opacity-80 uppercase tracking-widest whitespace-nowrap">NIGHT TARGET</p><p className="text-2xl font-black leading-none">{animal.targetNightTemp || 22}°C</p></div>
                                             </div>
                                         </div>
                                         {(animal.targetHumidityMin || animal.targetHumidityMax) && (
                                            <div className="bg-cyan-600 text-white p-3 rounded-xl shadow-lg flex items-center gap-3">
                                                 ...
                                                 <div className="min-w-0"><p className="text-[8px] font-black opacity-80 uppercase tracking-widest whitespace-nowrap">HUMIDITY RANGE</p><p className="text-2xl font-black leading-none">{animal.targetHumidityMin || '?'}-{animal.targetHumidityMax || '?'}%</p></div>
                                            </div>
                                         )}
                                     </div>
                                </div>
                        ...
                        <>
                            ...
                            <div className="flex-1 py-8 pl-5 pr-8 grid grid-cols-12 gap-6 overflow-hidden">
                                    ...
                                    {mapLayout === 'side' ? (
                                        ...
                                        <>
                                            <div className="flex flex-col gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200 shadow-sm">
                                                ...
                                                <div className="flex items-center gap-3"><div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-slate-500 shadow-sm border border-slate-200 shrink-0"><Calendar size={16}/></div><div><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">ARRIVED</p><p className="font-bold text-slate-800 text-sm">{getArrivalYear(animal.arrivalDate)}</p></div></div>
                                            </div>
                                ...
                                <div className={`col-span-8 flex flex-col h-full ${mapLayout === 'bottom' ? 'gap-3' : 'gap-6'}`}>
                                        ...
                                        {mapLayout === 'side' && (
                                            <>
                                                <div className="flex gap-8 mb-6 mt-6 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                                    ...
                                                    <div className="flex items-center gap-3"><div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-slate-500 shadow-sm border border-slate-200"><Calendar size={20}/></div><div><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">ARRIVED</p><p className="font-bold text-slate-800">{getArrivalYear(animal.arrivalDate)}</p></div></div>
                                                </div>
                                    ...
                                    {mapLayout === 'bottom' && <div className="h-72 bg-slate-50 rounded-2xl p-4 border-2 border-slate-200 shadow-sm shrink-0 flex flex-col mb-6"><h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.25em] w-full text-left mb-2 pl-1">NATIVE RANGE</h3><div className="rounded-xl overflow-hidden border border-slate-200 w-full bg-white relative flex items-center justify-center flex-1">{animal.distributionMapUrl ? <img src={animal.distributionMapUrl} alt="Range Map" className="w-full h-full object-contain" crossOrigin="anonymous"/> : <div className="w-full h-full flex items-center justify-center text-slate-300"><Globe size={48} /></div>}</div></div>}
                                </div>
                            </div>
                            <div className="h-32 bg-[#10b981] flex items-center justify-between px-10 text-white relative overflow-hidden shrink-0"><div className="absolute inset-0 opacity-10 pointer-events-none" style={{ backgroundImage: 'radial-gradient(circle, #fff 2px, transparent 2.5px)', backgroundSize: '24px 24px' }}></div><div className="relative z-10 max-w-[70%]"><h2 className="text-3xl font-black uppercase italic tracking-wide mb-1 shadow-black drop-shadow-sm">ADOPT {animal.name} TODAY!</h2><p className="text-sm font-medium opacity-95 leading-snug">Scan the code to adopt {animal.name}. Your support helps provide food, care, and enrichment for our collection.</p></div><div className="relative z-10 bg-white p-2 rounded-xl shadow-2xl shrink-0"><img src={qrCodeUrl} alt="Adoption QR" className="w-24 h-24" crossOrigin="anonymous"/></div></div>
                        </>
                    )}
                </div>
            </div>
