...
import { 
    ...
    Heart, AlertTriangle, ShieldCheck, PenTool, Loader2
} from 'lucide-react';
...
const DailyRounds: React.FC = () => {
    // FIX: Get all data from useAppData context instead of props.
    const { animals, currentUser, addSiteLog, addIncident } = useAppData();
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-64px)] md:h-auto bg-slate-100 animate-in fade-in duration-300">
            ...
            <div className="bg-white border-t-2 border-slate-200 p-4 fixed bottom-0 left-0 md:left-64 right-0 z-30 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]"><div className="max-w-4xl mx-auto space-y-4"><div><div className="flex justify-between items-end mb-1"><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{activeTab} Section Progress</span><span className={`text-xs font-black ${isComplete ? 'text-emerald-600' : 'text-slate-800'}`}>{completedChecks} / {totalAnimals} Checked</span></div><div className="h-2 bg-slate-100 rounded-full overflow-hidden"><div className="h-full bg-emerald-500 transition-all duration-300" style={{ width: `${progress}%` }}></div></div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div className="space-y-2"><div className="flex gap-2"><input type="text" placeholder="Your Initials" maxLength={3} value={signingInitials} onChange={(e) => setSigningInitials(e.target.value.toUpperCase())} className="w-16 bg-slate-50 border-2 border-slate-200 rounded-xl px-2 py-3 text-center text-sm font-black uppercase focus:outline-none focus:border-slate-400 transition-colors" /><input type="text" placeholder={isNoteRequired ? "MANDATORY: Why were waters skipped?" : "Section Notes..."} value={generalNotes} onChange={(e) => setGeneralNotes(e.target.value)} className={`flex-1 border-2 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none transition-colors ${isNoteRequired ? 'bg-amber-50 border-amber-300 focus:border-amber-500 placeholder-amber-400 text-amber-800' : 'bg-slate-50 border-slate-200 focus:border-slate-400'}`}/></div>{currentUser?.signature && signingInitials && (<div className="flex items-center gap-2 px-3 py-2 bg-slate-50 rounded-xl border border-slate-200 opacity-80"><PenTool size={12} className="text-slate-400"/><span className="text-[8px] font-black text-slate-400 uppercase tracking-widest mr-2">DIGITAL SIG:</span><img src={currentUser.signature} alt="Sig" className="h-6 w-auto mix-blend-multiply" /></div>)}</div><button onClick={handleSignOff} disabled={!isComplete || isSubmitting || !signingInitials || isNoteRequired} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black uppercase text-xs tracking-widest shadow-lg flex items-center justify-center gap-2 hover:bg-black transition-all disabled:bg-slate-300 disabled:shadow-none disabled:cursor-not-allowed active:scale-95 h-full">
                {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : <ShieldCheck size={18}/>} Verify & Sign Off {activeTab}</button></div></div></div>
